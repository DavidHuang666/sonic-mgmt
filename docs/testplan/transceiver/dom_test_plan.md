...
| rx_power_consistency_variation_threshold | float | 3.0 | O | transceivers or platform | Maximum allowed absolute delta between two consecutive per-lane Rx power samples during DOM data consistency verification (`abs(curr_dBm - prev_dBm)`; threshold effectively in dB) |
| tx_bias_consistency_variation_threshold | float | 10.0 | O | transceivers or platform | Maximum allowed per-lane Tx bias variation between two consecutive samples during DOM data consistency verification, expressed as percent change versus the previous sample |
...
**Consistency variation threshold rule:** Consistency variation threshold attributes are optional and enable parameter-specific checks during DOM data consistency verification. If a threshold attribute is absent from the resolved `DOM_ATTRIBUTES` for a port, skip that parameter's consistency check for that port. Tx bias uses percentage-based variation because the normal bias current magnitude can vary widely across module classes, so a single absolute mA delta does not generalize well. Compute Tx bias percent change as `abs(curr_mA - prev_mA) / abs(prev_mA) * 100` only when the lane's Data Path State Machine is `DPActivated` and both Tx bias samples are numeric and within the normal warning range defined by `tx_bias_threshold_range` (`lowwarning <= tx{lane}bias <= highwarning`). If the Data Path State Machine is not `DPActivated`, the warning range cannot be evaluated, either sample is non-numeric, or either sample is outside the normal warning range, skip that lane/sample-pair's Tx bias percentage check and log the skip.
